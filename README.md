...
## Length: the concept decides

The window is **25–45 seconds** (62–112 words), in `schema.py`, delivered as **4–5
answer beats of at most 24 words each**. There is deliberately **no target inside
that range** — `script.py` states bands and lets the concept pick one:

| band | seconds | what it is |
|---|---|---|
| 62–75 words | ~25–30s | a concise but complete concept — a finished short, not a short one |
| 75–95 words | ~30–38s | the normal case: a mechanism and its consequence |
| 95–112 words | ~38–45s | a richer concept, with a grounded example and/or a misconception |

**The window has moved four times, and only the last move has evidence.** 30–60 →
18–45 → 12–28 → 35–50 → 25–45. The two early cuts were both fixing the same
defect: a floor applied to a topic that did not have the content to fill it, so the
extra beat was always the weakest — a restatement, or a claim propped up by a
citation that did not really support it. E016 freezes one of those padded scripts.

35–50 was raised on the theory that length was never the problem — that a
45-second short is only padded when the *topic* could not fill it, and that a depth
gate in `select` plus an understanding-driven plan would make the length honest.
**The theory was half right and the floor was still wrong.** Four real runs against
`css_specificity.md`, a document written specifically to be rich enough for 45
seconds, produced scripts of 67, 77 and 71 words — mean 28.7 seconds — with most
attempts passing every other grader. One opened by correcting the misconception,
scored both of the section's real selectors verbatim, and landed the consequence.
It was a good short whose only fault was being 28 seconds long.

So the natural length of a source-grounded, one-idea-per-beat explanation of a
single concept is about 28–30 seconds, and three separate settings of this window
have now produced that number. The floor is 25s to admit it.

**What tells a good short from a padded one is not the floor**, and this is the
whole point. Two graders do that job and neither was touched:

- `MAX_ANSWER_WORDS` stays at **24**, so a beat is always one idea.
- `check_beats_develop` rejects a beat that is mostly words already heard.

E045 is the proof: a padded **32-second** short — inside the window — rejected on
repetition, not on length. Its partner E049 is a real **29-second** short asserted
to pass every grader. Together they say the thing the window change rests on.

`plan_depth` enforces the other side, before a word is written: a plan is deep
enough exactly when it holds `MIN_ANSWERS` things to say (a teaching step, an
example the reading says to use, a misconception it says to correct). Three things
asked for four beats has to pad the fourth, and that topic belongs to select's
gate rather than to a longer script.

Both sample documents now work: `session_18_paging.md` (sections of 47–78 words)
fits the concise band, and `content/css_specificity.md` (123–138 words per section,
each with a literal worked example and a correctable misconception) reaches the
richer one. The free offline run points at the latter:
