...
teaching_sequence — the SMALLEST sequence of steps a beginner has to be walked
  through to end up understanding core_idea. Two to four steps. Each one is an
  object with three fields, and they are three different questions:
...
confusion_plan — whether THIS short should correct a wrong belief, and which one.
  ...
  "not_needed" IS THE ORDINARY ANSWER. Choose it whenever the section is simply
  explaining something, which is most of the time. Correcting a misconception is
  powerful and it is not free: it costs one of two or three beats, and it plants
  the wrong belief in the viewer's head in order to knock it down. That is only
  worth doing when the belief was already there. A short that warns against a
  mistake nobody makes has spent its best beat teaching an error.
