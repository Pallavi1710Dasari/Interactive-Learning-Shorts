...
#:
#: Tied to the shape of the thing being planned, not picked for roundness: the
#: script is 2-3 answer beats (checks.MAX_ANSWERS), so a five-step sequence is
#: already planning an explanation the short cannot deliver, and the beats would
#: have to drop steps — silently, and by whichever ones the model found hardest to
#: say. One step over the beat count is the useful slack, because a step can be a
#: setup that shares a beat with the one after it.
MAX_TEACHING_STEPS = 4
...
def check_teaching_sequence(understanding, section_text: str | None = None) -> GraderResult:
    ...
    Four questions, in the order they are worth asking:
      ...
      1. IS THERE ONE, and is it short enough to be a plan for 2-3 beats rather
         than a syllabus.
...
#: sequence is a plan for explaining and the script is what fits in 2-3 beats: a
...
#: has already heard, in a short with two or three beats to spend.
#:
